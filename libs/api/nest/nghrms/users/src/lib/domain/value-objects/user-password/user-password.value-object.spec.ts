...
describe('password.value-object', () => {
  it('should create a valid password', () => {
    const password = UserPassword.create({ value: '123abc' });
  ...
  it('should fail if password is not on range min 3 char and max 20 char', async () => {
    const shortPassword = await UserPassword.create({ value: 'i' });
    ...
    const longPassword = UserPassword.create({
      value: 'invalid_long_password_to_validate_password_must_have_max_20_char',
    });
